...
## 9. `fec_calendar` — three fixes verified against live data (updated July 2026)

- **`/election-dates/` uses `min_election_date`/`max_election_date`, not `min_date`/`max_date`.** `fec_calendar`'s `election_dates` mode maps the tool-facing `min_date`/`max_date` params onto these internally — confirmed live: `mode=election_dates, election_year=2026, min_date=2026-11-01, max_date=2026-11-10` returns exactly 91 results, all `election_date: "2026-11-03"` (not all 242 records for the year). If a future change to this mapping regresses, that's the symptom to watch for.
- **`/reporting-dates/` (filing_deadlines mode) has no filer-level discriminator field** — a raw upstream query for 2026 H2 deadlines returns 243 rows that are exact duplicates in groups keyed by `(due_date, report_type, report_type_full, report_year)`, with nothing else to distinguish individual filers. `fec_calendar` now walks every upstream page, collapses each group to one row, and adds a `filer_count` field (raw rows collapsed into that row) rather than silently dropping the duplication. Confirmed live: 243 raw rows → 35 deduped rows; `due_date=2026-12-03, report_type=30G` → `filer_count=92` (nearly every filer type shares a general-election-tied deadline); `due_date=2026-10-15, report_type=Q3` → `filer_count=1`. Dedup ratio scales with how broadly a report_type applies — general-election-tied types (30G, 12G, etc.) dedupe highest since almost all filers share election day; narrow quarterly/monthly types dedupe least. `pagination.count` in the tool's response reflects the deduped count (35), not the raw upstream count (243).
- **Date field format now normalized across modes.** `election_dates` mode's upstream `create_date`/`update_date` come back as full ISO timestamps (e.g. `2026-01-21T14:48:27`); `filing_deadlines` mode's upstream fields come back as bare `YYYY-MM-DD`. Both are now normalized to bare `YYYY-MM-DD` in the tool's output. Note: `events` mode (`/calendar-dates/`) doesn't return `create_date`/`update_date` at all — confirmed live, no such fields appear on any event row — so there was nothing to normalize there.

## 10. `fec_committee_reports`'s `most_recent=true` doesn't filter server-side (fixed September 2026)

Confirmed live against a committee with a long amendment history (Kamala Harris for President, `C00694455`): calling `/committee/{id}/reports/?most_recent=true` for a single reporting period still returned every historical amendment of that report — 20 rows for one period, 8 of them still individually flagged `most_recent: true`. The parameter doesn't reduce the result set at all; it's the per-row `most_recent` boolean that's reliable, not the query param. `committeeReports.ts` now filters client-side (`results.filter(r => r.most_recent !== false)`) whenever `is_amended` is left unset, which is the case meant to mean "current version of each report only." If you see duplicate periods coming back from this tool in the future, this is the first place to check.

**Separately, live schedule_a/schedule_b field names differ from what OpenFEC's Python source implies for one specific documented item:** the Schedule B aggregate models' memo-exclusion column is named plain `total` on the live API (`ScheduleBByPurpose`, `ScheduleABySize`), not `non_memo_total` as `docs.py`'s prose suggested — `total` still excludes memo rows, it's just not the literal field name. See `docs/analysis/implementing-the-primer.md` §3 for the full verification writeup this came out of.

**Also confirmed: even a `min_date`/`max_date`-bounded Schedule A/B query can still time out without an explicit `sort` param** on a high-volume committee (DNC, RNC) — adding `sort=contribution_receipt_date` (or `disbursement_date`) alongside the date bound resolved it. If a bounded query still 504s, try adding sort before assuming the date range needs to be narrower.
